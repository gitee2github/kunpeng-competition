...
@jsonschema.FormatChecker.cls_checks("cron")
def validate_cron_format(entry):
    try:
        croniter.croniter(entry)
        return True
...
@jsonschema.FormatChecker.cls_checks("timezone")
def validate_timezone_format(entry):
    try:
        pytz.timezone(entry)
        return True
    except Exception:
...
def validate_posix_path(entry):
    ...
    res = re.match(r"^(/([A-Z]|[a-z]|[0-9]|\-|_)+)+$", entry)
    return res is not None
