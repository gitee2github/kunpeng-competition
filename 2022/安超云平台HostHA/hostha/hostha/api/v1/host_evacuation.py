...
from hostha.api.v1.schemas.host_evacuation import HOST_EVACUATION_SCHEMA
...
rest = u.RestV2("host_evacuation", __name__)


@rest.get("/host_evacuation")
@acl.enforce("hostha:host_evacuation:get")
@v.validate(HOST_EVACUATION_SCHEMA)
def host_evacuation_get(data):
    ctx = context.ctx()
    filters = {}
    if 'hostname' in data:
    ...
    records = db_api.ha_host_evacuation_get_by_filters(ctx, filters)
